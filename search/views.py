...
def search(request):
    ...
    if search_query:
        ...
        if _format == "json":
            ...
            results["documentation"] = sort_alphabetically(
                results["documentation"], reverse=True, sort_by="type"
                )
           
            results["git"] = sort_alphabetically(results["git"])
            results["packages"] = sort_alphabetically(results["packages"])
            results["forum"] = sort_alphabetically(
                results["forum"], sort_by="message"
                )
